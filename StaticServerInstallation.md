...
!> 前排提示，静态服务端有门槛要求：如果下面这些概念里面有任何一个你没有听说过的，那么请转而使用PHP服务端或者单文件服务端，因为静态服务端不适合你（如果是大佬请忽略）

!> 再次提示，使用静态服务端请务必仔细阅读教程，否则会有误删文件的风险！否则会有误删文件的风险！否则会有误删文件的风险！如阅读不仔细导致误删文件，请自己承担后果！

1. 对象存储和对应的访问秘钥（ID和KEY）
2. 常见超文本传输协议错误代码
3. 端口映射
4. YAML格式
5. 超文本传输协议之统一资源定位器
...
7. 上传完成后，请在浏览器里访问`index.yml`文件的URL，如果一切顺利，浏览器会显示`index.yml`文件的内容。如果出现了403错误，那多半是桶的权限没有设置为公有读私有写。其它情况请根据厂商文档进行排查
