...
`影响范围`：Jar客户端4.1.10及更高

`问题原因`：目前未知

`解决方法`：升级到4.1.14版本或更高。然后在配置文件里把主题禁用即可`disable-theme: true`

### 连接中断

`影响范围`：Jar客户端4.1.10及更高

`问题原因`：由于网络原因，已建立的TCP连接被意外断开
...
`问题原因`：由于网络原因，服务器未能在指定时间内及时应答客户端的TCP数据包。

`解决方法`：这不是程序bug，请检查客户端与服务器之间的网络是否稳定

一个特例是：如果超时的请求文件为`res.json`并且你使用的是单文件服务端，请检查你的`res`目录体积是不是过大（300mb以上），如果是，请开启单文件服务端的高性能模式。如果你使用的是php服务端，请更换为静态服务端或者单文件服务端。
...
+ `配置文件config.yml`无法解码为`yaml`格式
+ `语言配置文件lang.yml`无法解码为`yaml`格式
+ `元数据文件请求`无法解码为`json`格式
+ `结构文件请求`无法解码为`json`格式
...
+ `配置文件config.yml`无法解码为`yaml`格式：自行检查配置文件的格式是否符合yaml规范
+ `语言配置文件lang.yml`无法解码为`yaml`格式：自行检查配置文件的格式是否符合yaml规范
+ `元数据文件请求`无法解码为`json`格式：自行检查服务端返回的`index.json`或者`index.php`文件的格式是否符合yaml规范
+ `结构文件请求`无法解码为`json`格式：向我报告此问题
...
`解决方法`：参考[HTTP状态码不正确](#HTTP状态码不正确)

### 由于安全机制或者IO错误，无法访问目录

`影响范围`：Jar客户端4.1.10及更高

`问题原因`：参考[安全机制导致操作失败](#安全机制导致操作失败)
...
`影响范围`：Jar客户端4.1.7至4.1.9

`问题原因`：1.把游戏客户端解压到了某个盘的根目录或者放到了某些系统文件夹里（比如AppData里）。2.需要更新的目录中包含无权限访问的目录

`解决方法`：1.不要解压到根目录上。2.放到有权限的目录中或者使用管理员权限运行。3.使用`base-path`选项避开访问无权限的目录其是使用静态服务端
