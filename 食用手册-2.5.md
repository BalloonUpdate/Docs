...
> **适用于（热更新包2.5.x，客户端2.1，动态服务端2.3，静态服务端2.4，小工具-d1.3-j1.0.1）**

> 创建于2021年1月13日，最后修改于2021年1月21日
...
8. 将需要更新的文件放到`resources`目录里（完整结构如下），即配置完成
9. ![file_structure_d](食用手册-2.5/file_structure_d.png)
...
11. 将需要更新的文件（如模组、资源包等）放到`resources`目录里，完整文件结构如下
12. ![file_structure_s](食用手册-2.5/file_structure_s.png)
13. 分别为`hotupdate`和`resources`生成**目录校验文件**，生成完毕后通常是5个文件
14. ![after-generating](食用手册-2.5/after-generating.png)
15. 将这5个文件上传到桶（Bukkit）的根目录
16. **将网站索引文件(默认首页)从 index.html 或者 index.php 更改为 index.json，**如果无法修改，请参考[这里](#十、无法修改网站索引文件)
17. 部署完成！
...
如果你需要部署到阿里云对象存储或者腾讯云对象存储，可以用这个小工具一键部署（可以免去生成校验文件的步骤），部署过程将变得非常容易
...
桶名：存储桶的名字，这个一般是创建的时候自己填写的

secret_id和secret_key：这个是账号的访问秘钥，每家服务商获取方式不相同，需要自己亲自去查询，或者询问我也可以（下方留言或者mcbbs帖子留言），建议使用子账户来提高安全性
