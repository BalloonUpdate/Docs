...
## 四、对象存储服务和FTP/SFTP的支持

小工具支持一键部署到对象存储服务和FTP，简单到直接把文件夹拖上去就好了，还不用手动生成校验文件
...
2. 如果使用FTP，请务必备份所有文件，避免误设置导致已有文件被误删

### 2. 配置文件介绍

#### service_provider

服务提供商，阿里云填写`aliyun`，腾讯云填写`tencent`，FTP填写`ftp`

#### skip_hashing_before_uploading

上传之前是否跳过生成校验文件的阶段（不生成校验文件就直接上传文件），默认请保持`false`

（小工具`1.6`以后已改名为`upload_without_hashing`）

#### 对象存储服务相关设置

小工具`1.6`以后为每个服务商都添加了单独的配置信息（配置隔离），只需要填写你需要的那一家服务商的信息就好了（小工具`1.5.x`版本请忽略）

##### bucket

桶名，存储桶的名字，这个一般是创建的时候自己填写的名字

（小工具`1.6`以后已改名为`bucket_name`）

##### secret_id和secret_key

别名`access_id`和`access_key`

用于API验证。建议使用子账户来提高安全性。
...
因为一些原因，SFTP无法校验FTP服务端证书，请自行确保FTP服务端的证书是有效的，此说明仅针对SFTP，FTP不受影响

FTP/SFTP上传完成后会在程序旁生成一个缓存文件`ftp.cache.json`，保存着FTP服务端的文件结构，用于快速对比文件差异实现增量上传，一般情况无需改动此文件，如果删除会进行一次全量上传（将所有文件重新上传一遍），请自行判断何时清理缓存文件

FTP协议无法直接返回文件校验信息，对文件内容的对比完全依赖于FTP服务端所返回的**文件修改时间**和**文件大小**，请确保服务端能正常返回这些信息，否则会导致新文件的内容无法正常上传
...
3. 这里填写的是原生正则表达式，不是纯文本路径，请注意半角点`.` 的转义和正则表达式的一些转义字符影响
...
!> 如果使用IIS HTTP 服务器，文件名带+号时可能会无法访问，请尽量避免此情况
...
>  2.4及以上版本支持集成到启动器，再也不用像原来那样需要运行更新器，检查更新完成后才弹出启动器，这实在是太难用了
...
本软件本身支持自升级，可以在每次启动时自动升级到服务端内的最新版本
...
因Windows机制所致，不能删除运行中的文件，所以无法对**客户端程序**`UpdaterClient-x.x.exe`文件自身进行更新，只能以重新发布完整MC客户端压缩包的形式来更新这个文件（这个文件BUG相对非常少，我也不会经常对这个文件进行更新）

## 十三、常见问题（QA环节）

### Q：使用HMCL时无法连接到更新服务器却能继续启动游戏

这种情况多发生于使用启动前指令启动本软件的场景下，因为HMCL机制的问题，无法正确地处理本程序传递出的错误（返回码1而非0），也就会继续运行了。不过此问题影响并不大。

由于HMCL机制所致，此问题暂时无解

### Q：弹框：出现异常: error

![error](食用手册-2.6/error.png)
